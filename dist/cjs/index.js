...
const TableRows = ({ record, column, config, columnNumber }) => {
    var _a, _b;
    const [showRowOptions, setShowRowOptions] = React.useState(false);
    const rowOptionsRef = React.useRef(null);
    const toggleOptions = () => {
...
const Table = ({ records, config, pageSize }) => {
    ...
    const [currentRecord, setCurrentRecords] = React.useState([]);
    const [completeRecord, setCompleteRecord] = React.useState(records || []);
    const [rowCount, setRowCount] = React.useState((records === null || records === void 0 ? void 0 : records.length) || 0);
    const [columnSortState, setColumnSortState] = React.useState({});
    const [columnNames, setColumnNames] = React.useState();
    const [itemPerPage, setItemPerPage] = React.useState(pageSize || 5);
    const [searchText, setSearchText] = React.useState("");
    const debounceSearch = useDebounce(searchText, 1000);
    const [theme, setTheme] = React.useState("light");
    const selectedRecord = React.useRef(null);
    const [totalPage, setTotalPage] = React.useState(Math.ceil((completeRecord === null || completeRecord === void 0 ? void 0 : completeRecord.length) / itemPerPage));
    ...
    const sortableColumns = React.useMemo(() => columns.filter((col) => (col === null || col === void 0 ? void 0 : col.sortable) === true).map((co) => co.id), [columns]);
    ...
    const searchableColumns = React.useMemo(() => columns.filter((col) => (col === null || col === void 0 ? void 0 : col.searchable) === true).map((co) => co.id), [columns]);
    ...
    const searchInTable = React.useCallback((e) => {
        var _a;
